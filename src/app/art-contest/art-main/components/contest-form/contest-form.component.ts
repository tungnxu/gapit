...
export class ContestFormComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if(changes.exams && this.exams){
      if (this.exams[0]) {
        this.firstExam = this.exams[0]
        this.submitContestForm.patchValue({
          exam_name:  this.firstExam.name_exam,
          description: this.firstExam.description,
          email: this.firstExam.email,
          file: this.firstExam.url
        });
      }
    }
  }
